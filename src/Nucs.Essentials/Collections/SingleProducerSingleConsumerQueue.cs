...
namespace Nucs.Collections {
    ...
    public sealed class SingleProducerSingleConsumerQueue<T> : IEnumerable<T> {
        ...
        private const int InitialSegmentSize = 512; // must be a power of 2
        ...
        public void Clear() {
            ...
            if (RuntimeHelpers.IsReferenceOrContainsReferences<T>()) {
                for (Segment? segment = _head; segment != null; segment = segment._next) {
                    if (segment is null)
                        continue;
                    Array.Clear(segment._array, 0, segment._array.Length);
                }
            } else {
                Array.Clear(_head._array, 0, _head._array.Length);
            }

            //very not performant way but quickest and safest to truely reset the state
            _head = _tail = _head is null ? new Segment(InitialSegmentSize) : new Segment(_head._array); //reuse head
        }
        ...
        public void Enqueue(ref T item) {
            ...
            // Slow path: there may not be room in the current segment.
            else EnqueueSlow(ref item, ref segment);
        }
        ...
        /// <param name="item">The item to enqueue.</param>
        /// <param name="segment">The segment in which to first attempt to store the item.</param>
        private void EnqueueSlow(ref T item, ref Segment segment) {
            Debug.Assert(segment != null, "Expected a non-null segment.");
        ...
        private sealed class Segment {
            ...
            public SegmentState _state; // separated out to enable StructLayout attribute to take effect
            ...
            public Segment(T[] array) {
                Debug.Assert((array.Length & (array.Length - 1)) == 0, "Size must be a power of 2");
                _array = array;
            }
        }
