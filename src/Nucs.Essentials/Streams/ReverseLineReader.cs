...
/// </summary>
public sealed class ReverseLineReader : IEnumerable<ReadOnlyMemory<char>> {
    /// <summary>
    ...
    /// <param name="filename">File to read from</param>
    /// <param name="encoding">Encoding to use to decode the file into text</param>
    public ReverseLineReader(string filename, Encoding encoding, int bufferSize)
        : this(() => File.OpenRead(filename), encoding, bufferSize) { }
    ...
    private IEnumerator<ReadOnlyMemory<char>> GetEnumeratorImpl(Stream stream) {
        try {
            ...
            while (position > 0) {
                ...
                for (int i = charsRead - 1; i >= 0; i--) {
                    ...
                    if (previousEnd == null) {
                        int start = i + 1;
                        var mem = new Memory<char>(charBuffer, start, endExclusive - start);
                        endExclusive = i;

                        if (!firstYield || mem.Length != 0) {
                            yield return mem;
                        }
                    } else {
                        int start = i + 1;
                        string bufferContents = new string(value: charBuffer, startIndex: start, length: endExclusive - start);
                        endExclusive = i;

                        string stringToYield = bufferContents + previousEnd;
                        if (!firstYield || stringToYield.Length != 0) {
                            yield return stringToYield.AsMemory();
                        }
                    }
            ...
            yield return previousEnd?.AsMemory() ?? default;
        } finally {
            stream.Dispose();
        }
    }
